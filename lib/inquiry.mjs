...
import { domainAcceptsMail } from './emailcheck.mjs';
...
export async function handleLead(payload, { ip }) {
  ...
  if (!(await domainAcceptsMail(email.split('@')[1]))) return { status: 400, body: { ok: false } };
...
export async function handleInquiry(payload, { ip, userAgent }) {
  ...
  else if (!(await domainAcceptsMail(email.split('@')[1]))) errors.push('Adres e-mail wygląda na błędny — ta domena nie przyjmuje poczty.');
